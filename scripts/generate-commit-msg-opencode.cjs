...
const readConfig = () => {
	...
	try {
		...
		return {
			opencodeModels: Array.isArray(parsed.opencodeModels)
...
const formatPromptFiles = (files) => {
	if (files.length <= MAX_PROMPT_FILES) return files.join("\n");

	return [
		...files.slice(0, MAX_PROMPT_FILES),
		`...and ${files.length - MAX_PROMPT_FILES} more staged files`,
	].join("\n");
};
...
if (config.fallbackToHeuristicGenerator) {
	try {
		execFileSync(
			"node",
			[
				path.join(process.cwd(), "scripts/generate-commit-msg.cjs"),
				messageFile,
			],
			{
				stdio: "ignore",
