...
## Ширина тестового покрытия для frontend

Для `site/assets/js/**` действует правило: coverage должен расти вместе с количеством контроллеров и пользовательских состояний.

- `npm test` - основной слой для controller logic, DOM sync, i18n updates, `aria-*` state, storage fallback и missing-DOM paths
- `npm run test:smoke` - быстрый сигнал, что сайт переживает базовый пользовательский путь
- `npm run test:e2e` - browser truth для reload persistence, CSS application, mobile behavior и других сценариев, которые нельзя надежно подтвердить в JSDOM

Что это означает practically:

- новый controller нельзя оставлять без focused unit tests
- smoke не заменяет глубокое покрытие preference/accessibility/initial-render логики
- при изменении controller contract нужно обновлять разметку, тесты и docs вместе
- по мере роста сайта тесты должны дробиться по модулям и critical flows, а не собираться в один общий spec

Browser-level growth policy:

- новый persisted preference flow должен получать browser test на reload persistence
- изменения theme/lang bootstrap в `<head>` должны проверяться browser-level, а не только в JSDOM
- mobile navigation changes должны иметь browser check на usability и `aria-*` state
- page-object pattern в `tests/e2e/pages/` нужно расширять вместе с ростом critical flows
...
После этого выполняется `required-checks` - агрегирующий check общего статуса site-pipeline.
...
## Lighthouse: регулярная quality-проверка

Для user-facing изменений сайта рекомендуется регулярный Lighthouse-check и цель `100/100/100/100` по всем четырем категориям.
...
Mobile `100` остается желаемой целью, но не считается blocker, пока нет regressions ниже этого baseline.

Практический минимум:

- запускать Lighthouse после заметных изменений layout, typography, hero/media, theme/lang logic, navigation и accessibility state
- отдельно смотреть mobile viewport, а не только desktop
- исправлять regressions до `git push`, если Lighthouse показывает проблему, которую можно надежно воспроизвести локально

Подробная памятка с чек-листом и рекомендациями лежит в `docs/lighthouse.md`.

Если нужен формальный BMAD-style review без story artifact, смотри `docs/bmad-review-without-story.md`.
