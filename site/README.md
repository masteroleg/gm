...
- деплой запускается только после успешного CI на `work`
...
3. GitHub Actions запускает полный CI
4. если CI зеленый, GitHub Pages получает новый сайт автоматически
...
Актуальность `site/assets/css/output.css` обеспечивается раньше - в `pre-commit`, где CSS пересобирается и автоматически добавляется в commit.
