...
- **S3 (fixed then reverted):** Previous review replaced `"Серіалізований запис одиниці"` with `"Запис одиниці з унікальним кодом"` citing Ukrainian Copy Rule. However, Epic 2 Epic-level review (2026-03-13) found this introduced a semantic mismatch — EN says "Serialized unit record" (format), UK said "record with unique code" (property). Reverted to `"Серіалізований запис одиниці"` which is idiomatic Ukrainian and semantically equivalent to EN. (`site/assets/js/lang-toggle.js`)
...
- 2026-03-13: Epic 2 code review (cross-story) — reverted S3 `verification.fact1Title` UK from `"Запис одиниці з унікальним кодом"` back to `"Серіалізований запис одиниці"` (semantic equivalence with EN restored); `verification.supportLabel` UK updated accordingly; `site/v/genuim/index.html` CSS cache version updated to `?v=19`.
