...
(function () {
  ...
  const PATINA = 'oklch(70% 0.12 188)';
  const PATINA_PALE = 'oklch(82% 0.07 188)';
  const PATINA_SOFT = 'oklch(70% 0.12 188 / 0.28)';
  const PATINA_FAINT = 'oklch(70% 0.12 188 / 0)';
  const PATINA_45 = 'oklch(70% 0.12 188 / 0.45)';
  const PATINA_GLOW = 'oklch(70% 0.12 188 / 0.18)';
  const PATINA_PALE_55 = 'oklch(82% 0.07 188 / 0.55)';
  const PATINA_HOVER_WASH = 'oklch(78% 0.12 82 / 0.12)';
  const NEUTRAL_91 = 'oklch(91% 0 0)';
  const NEUTRAL_72 = 'oklch(72% 0 0)';
  const NEUTRAL_58 = 'oklch(58% 0.15 35)';
  const NEUTRAL_49 = 'oklch(49% 0.08 188)';
  const NEUTRAL_45 = 'oklch(45% 0.18 145)';
  const NEUTRAL_34 = 'oklch(34% 0 0)';
  const SHADOW_LOW = '0 4px 16px oklch(0% 0 0 / 0.16), 0 1px 3px oklch(0% 0 0 / 0.1)';
  const SHADOW_LOW_HOVER = '0 7px 22px oklch(0% 0 0 / 0.18), 0 2px 5px oklch(0% 0 0 / 0.12)';
  ...
  const CONFIGURE_PILL_RADIUS = '999px';
  const CONFIGURE_SELECTION_PILL_BORDER = '1px solid ' + PATINA;
  ...
  function initEditBadge() {
    ...
    // Preserve keyboard visibility with a custom focus ring that fits the badge.
    if (!uiGetById(PREFIX + '-edit-badge-focus-style')) {
      const s = document.createElement('style');
      s.id = PREFIX + '-edit-badge-focus-style';
      s.textContent =
        '#' + PREFIX + '-edit-badge button { box-shadow: 0 2px 8px rgba(0,0,0,0.1) !important; }' +
        '#' + PREFIX + '-edit-badge button:focus-visible {' +
        '  outline: 2px solid ' + ACCENT + ';' +
        '  outline-offset: 2px;' +
        '  box-shadow: 0 0 0 3px ' + P.accentSoft + ', 0 2px 8px rgba(0,0,0,0.1) !important;' +
        '}' +
        '[data-impeccable-editable="true"] { outline-offset: 2px; }' +
        '[data-impeccable-editable="true"]:focus-visible {' +
        '  outline: 2px solid ' + ACCENT + ';' +
        '  outline-offset: 2px;' +
        '  box-shadow: 0 0 0 3px ' + P.accentSoft + ';' +
        '}';
      uiAppendStyle(s);
    }
  }
...
void main() {
  ...
  function initPageChat(parent, P) {
    ...
    if (!uiGetById(PREFIX + '-page-chat-style')) {
      ...
      s.textContent =
        ...
        '@keyframes impeccable-steer-processing { 0%, 100% { border-color: ' + PATINA_SOFT + '; box-shadow: 0 0 0 0 ' + PATINA_FAINT + '; } 50% { border-color: ' + PATINA_PALE_55 + '; box-shadow: 0 0 14px ' + PATINA_GLOW + '; } }' +
        ...
        '@media (prefers-reduced-motion: reduce) { #' + PREFIX + '-page-chat[data-processing="true"] { animation: none; border-color: ' + PATINA_45 + '; } #' + PREFIX + '-page-chat[data-processing="true"] [aria-hidden="true"] span { animation: none; opacity: 0.85; } }' +
        '#' + PREFIX + '-page-chat[data-voice-listening="true"] { border-color: ' + PATINA_45 + '; }' +
        ...
        '#' + PREFIX + '-page-chat-input::placeholder { color: ' + NEUTRAL_72 + '; opacity: 1; }' +
        '#' + PREFIX + '-page-chat-input { caret-color: ' + C.brand + '; }' +
        '#' + PREFIX + '-page-chat[data-input-focused="true"]:not([data-expanded="true"]) #' + PREFIX + '-page-chat-input::placeholder { color: ' + NEUTRAL_72 + '; }' +
        '#' + PREFIX + '-page-chat-voice:hover { background: ' + PATINA_HOVER_WASH + '; }';
      uiAppendStyle(s);
    }
  ...
  function initGlobalBar() {
    ...
    // Custom focus-visible for bar buttons. Keep keyboard focus visible,
    // but tune the ring to the bar palette.
    if (!uiGetById(PREFIX + '-bar-focus-style')) {
      const s = document.createElement('style');
      s.id = PREFIX + '-bar-focus-style';
      s.textContent =
        '#' + PREFIX + '-global-bar button:focus { outline: 2px solid ' + P.accent + '; outline-offset: 2px; }' +
        '#' + PREFIX + '-global-bar button:focus-visible {' +
        '  outline: 2px solid ' + P.accent + ';' +
        '  outline-offset: 2px;' +
        '  box-shadow: 0 0 0 2px ' + P.accentSoft + ';' +
        '}' +
    ...
    const designBtn = makeIconBtn({
      ...
      svg: `<span style="display:inline-grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;width:14px;height:14px;border-radius:4px;overflow:hidden;box-shadow:inset 0 0 0 1px oklch(92% 0 0 / 0.13);flex-shrink:0">
        <span style="background:${C.brand}"></span>
        <span style="background:${PATINA}"></span>
        <span style="background:${NEUTRAL_91}"></span>
        <span style="background:${NEUTRAL_34}"></span>
      </span>`,
      label: 'DESIGN.md',
    ...
    pendingPillEl.addEventListener('mouseenter', () => {
      ...
      pendingPillEl.style.boxShadow = SHADOW_LOW_HOVER;
    });
    pendingPillEl.addEventListener('mouseleave', () => {
      if (pendingApplyInFlight) return;
      pendingPillEl.style.filter = 'none';
      pendingPillEl.style.transform = 'scale(1)';
      pendingPillEl.style.boxShadow = SHADOW_LOW;
    });
    ...
    const showTrashTooltip = () => {
      ...
      pendingTrashBtn.style.boxShadow = SHADOW_LOW_HOVER;
