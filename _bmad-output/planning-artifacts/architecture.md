...
29 active Phase 1 FRs + 3 roadmap notes:
...
- Proof Example & Demo Input (FR10–FR19, FR25): статичні proof-pages Phase 1; live lookup — Phase 3 roadmap
- SEO Intercept & Lead Capture (FR21–FR24): request/contact flow через `mailto:` UX handoff у Phase 1; без server-side request ingestion
- Knowledge Base & Trust Surface (FR26–FR27): статичні сторінки; knowledge base — IA reservation тільки
- Measurement & Enablement (FR29–FR31): release-review visibility без backend analytics pipeline у межах цього pass
- Roadmap-only notes: `FR20` (Phase 2+), `FR28` (Phase 2), `FR32` (Phase 3) — не входять в implementation-ready Phase 1 coverage
...
- **Request/contact handoff Phase 1:** `mailto:` UX handoff mechanism for the approved 5-field request form; це не надійний transport layer і не server-side ingestion path
...
- **Azure migration benefit:** custom HTTP headers (CSP, NFR14) і кращий deployment control; цей документ не визначає жоден backend request capture path
...
- `request-form.js` — contact handoff controller: `mailto:` UX handoff + no-mail-client fallback + URL param prefill (`?scenario=`)
...
- **Request/contact handoff Phase 1:** `mailto:` is a UX handoff mechanism, not a reliable transport or backend capture path
- **Request metadata in Phase 1:** `scenario`, `source_path`, і опційний `proof_path` передаються best-effort у generated `mailto:` payload; поведінка environment-dependent, бо `mailto:` контролюється клієнтом
- **No-mail-client fallback:** якщо `mailto:` не відкривається або в користувача немає налаштованого mail client, UI не стверджує, що request submitted; введені поля залишаються доступними на сторінці, а користувач отримує явний contact fallback / next step без блокування навігації
- **Phase boundary:** цей документ не визначає backend request capture, request logging, telemetry pipeline, або server-side ingestion path для Phase 1
...
- **Component rendering:** Pure static HTML для approved Phase 1 proof surfaces; будь-яке розширення публічного proof model beyond approved Phase 1 states залишається roadmap-only
...
- **Phase 2+:** Azure Web App
  ...
  - Azure benefit: custom HTTP headers and stronger hosting controls; request capture architecture is intentionally not defined in this pass
...
5. Branch cards + `request-form.js` (contact handoff)
...
- `mailto:` contact handoff ізольований від решти сайту; fallback behavior має бути локальним для request surface і не вводить server-side залежностей
...
│   ├── request-form.test.js          # [NEW] mailto handoff, no-mail-client fallback, URL param prefill, validation
│   └── e2e/
│       ├── pages/
...
│       └── form.spec.js              # [NEW] contact handoff, no-mail-client fallback, URL param prefill
...
│       │   └── request-form.js       # [NEW] contact handoff + no-mail-client fallback + URL param prefill
│       ├── fonts/
...
- Request/contact flow: `mailto:` UX handoff only; без server-side request ingestion
...
- `site/assets/js/request-form.js` — contact handoff + `?scenario=` prefill
...
**FR26–FR27, FR29–FR31 (Knowledge Base, Trust Surface, and Review Visibility):**
...
- Form/contact handoff: `mailto:info@genu.im?subject=...&body=...` — browser-native UX handoff; best-effort metadata only; no backend transport promised
...
User lands on page
  ...
  → User clicks CTA → request-form.js reads ?scenario= and page context → builds `mailto:` payload with `scenario`, `source_path`, and optional `proof_path`
  → If the environment supports `mailto:` → native mail client opens with best-effort preserved metadata
  → If `mailto:` is unavailable or fails → entered values stay visible, UI shows explicit contact fallback, and the user is not told the request was submitted
```
...
Усі 29 active Phase 1 FR покриті архітектурними рішеннями; `FR20`, `FR28`, і `FR32` залишені як roadmap-only notes поза implementation-ready coverage:
...
- FR26–FR27: `site/knowledge/`, `site/about/`, `site/faq/`, article-template.html
- FR29–FR31: release-review visibility and request/proof path terminology across public surfaces; без backend analytics pipeline у межах цього pass
...
- [x] Project context thoroughly analyzed (29 active Phase 1 FR + 3 roadmap notes + 16 NFR)
...
- [x] Integration patterns defined (`mailto:` UX handoff Phase 1; no backend request ingestion in scope)
...
- [x] Requirements to structure mapping complete (6 active FR groupings)
...
- Proof page static HTML model — обґрунтований вибір для approved Phase 1 proof surfaces без scope drift у roadmap-only directions

**Areas for Future Enhancement:**
- Per-page OG images (Phase 2)
- Automated sitemap generation (Phase 2+)
- NewsArticle schema + `/news/` контент (Phase 2+)
- Roadmap notes `FR20`, `FR28`, `FR32` залишаються поза implementation-ready Phase 1 backlog
...
6. request-form.js (contact handoff + fallback)
7. sitemap.xml + robots.txt + og-default.png
