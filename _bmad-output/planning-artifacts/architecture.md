...
- Proof Example & Demo Input (FR10–FR20): статичні proof-pages Phase 1; live lookup — Phase 3
- SEO Intercept & Lead Capture (FR21–FR25): форма — mailto-заглушка Phase 1; source-path через URL params у hidden fields
...
- **Form submission Phase 1:** mailto-заглушка — мінімум зусиль, не шкода викинути при переході на Azure
...
- **Azure migration benefit:** custom HTTP headers (CSP, NFR14), form backend (Azure Functions), source-path metadata — вирішуються нативно
- **URL routing:** /v/genuim та інші proof pages → окремі static HTML files
...
- **Security headers Phase 1:** GitHub Pages не підтримує HTTP headers; fallback через `<meta http-equiv>` для CSP
- **URL params prefill:** ?sector= читається RequestForm-контролером при init
...
- `request-form.js` — mailto fallback + URL param prefill (?sector=)
...
- Full HTTP security headers → Azure migration
...
- **Proof page data model:** Pure static HTML — контент закодований в HTML файлах. Нуль JS overhead для рендерингу. Виправданий для 3 фіксованих sample-сторінок Phase 1.
...
- **Security headers Phase 1:** `<meta http-equiv>` fallback для CSP (GitHub Pages не підтримує HTTP headers)
- **Security headers Phase 2+:** Повноцінні HTTP headers після Azure migration
- **Known limitation:** meta-tag CSP не покриває всього що HTTP header покриває; зафіксовано як acceptable risk для Phase 1 статичного сайту

### API & Communication Patterns

Немає API Phase 1.

- **Form submission Phase 1:** `mailto:` заглушка — мінімум зусиль, легко замінюється при Azure migration
- **Source-path metadata:** URL params (`?sector=`, `?source=`) у mailto href
...
- **Component rendering:** Pure static HTML для proof pages (Phase 1); JS-rendered з data object — Phase 2+ при >3 sample pages
- **URL param prefill:** `?sector=` читається `request-form.js` при init (300ms delay)
...
**Заборонено:**
- Proof page без demo banner (Phase 1)
...
- URL params: `?sector=`, `?source=` читаються тільки на client-side
...
**FR10–FR20 (Proof Pages & Demo Input):**
...
- `site/assets/js/request-form.js` — mailto + `?sector=` prefill
...
User lands on page
  ...
  → User clicks CTA → request-form.js reads ?sector= → builds mailto href
...
- FR10–FR20: `site/v/`, `verification-page.js`, static HTML model, demo-banner pattern
- FR21–FR25: `request-form.js`, sitemap.xml, robots.txt, og-default.png
...
- Security (NFR12–NFR14): `<meta http-equiv>` Phase 1; HTTP headers після Azure
...
- Automated sitemap generation (Phase 2+)
- NewsArticle schema + `/news/` контент (Phase 2+)
- Full HTTP security headers (після Azure migration)
